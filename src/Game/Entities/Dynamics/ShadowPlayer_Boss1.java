...
public class ShadowPlayer_Boss1 extends BaseHostileEntity implements Fighter{

    Rectangle shadowPlayer;
    int width, height;

    public ShadowPlayer_Boss1(Handler handler, int xPosition, int yPosition, String state, String name, String area, BufferedImage[] animFrames) {
    ...
    public void render(Graphics g) {
        ...
        if(handler.getArea().equals(this.Area)) {
            if (!Player.checkInWorld) {
                shadowPlayer = new Rectangle((int) (handler.getXDisplacement() + getXOffset()),
                        (int) (handler.getYDisplacement() + getYOffset()), 45, 45);

            } else {
                shadowPlayer = new Rectangle((int) (handler.getXInWorldDisplacement() + getXOffset()),
            ...
            g.drawImage(Images.ghost,shadowPlayer.x,shadowPlayer.y,shadowPlayer.width,shadowPlayer.height,null);

            if (shadowPlayer.intersects(handler.getEntityManager().getPlayer().getCollision())) {
    ...
    double health=200,mana=50,xp=0l,lvl=2,defense=24,str=16,intl=30, mr = 10,cons=20,acc=10,evs=2,initiative=10, maxHealth = 200;
    String Class = "none",skill = "Fire";
